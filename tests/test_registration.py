...
class TestRegistration:
    ...
    def test_registration_valid_unique_credentials(self, driver: WebDriver, unique_credentials: dict[str, str]):
        ...
        WebDriverWait(driver, 5).until(ec.element_to_be_clickable(DL.POP_BUTTON_NO_ACCOUNT))
        driver.find_element(*DL.POP_BUTTON_NO_ACCOUNT).click()
        ...
        driver.find_element(*DL.POP_INPUT_EMAIL).send_keys(unique_credentials['email'])
        driver.find_element(*DL.POP_INPUT_PASSWORD).send_keys(unique_credentials['password'])
        driver.find_element(*DL.POP_INPUT_SUBMIT_PASSWORD).send_keys(unique_credentials['password'])
        # Загрузка кнопки "Создать аккаунт" в форме регистрации
        WebDriverWait(driver, 5).until(ec.element_to_be_clickable(DL.POP_BUTTON_CREATE_ACCOUNT))
    ...
    def test_registration_invalid_email(self, email, driver: WebDriver):
        ...
        WebDriverWait(driver, 5).until(ec.visibility_of_all_elements_located(DL.POP_INPUT_ERROR))
        WebDriverWait(driver, 5).until(ec.visibility_of_element_located(DL.POP_INPUT_ERROR_MESSAGE))
        inputs_with_errors = driver.find_elements(*DL.POP_INPUT_ERROR)
        input_error_message = driver.find_element(*DL.POP_INPUT_ERROR_MESSAGE).text 
    ...
    def test_registration_existing_credentials(self, password, driver: WebDriver):
        ...
        driver.find_element(*DL.POP_INPUT_SUBMIT_PASSWORD).send_keys(password)
        # Загрузка кнопки "Создать аккаунт" в форме регистрации
        WebDriverWait(driver, 5).until(ec.element_to_be_clickable(DL.POP_BUTTON_CREATE_ACCOUNT))
        driver.find_element(*DL.POP_BUTTON_CREATE_ACCOUNT).click()
